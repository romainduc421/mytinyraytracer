...
bool scene_intersect(const Vec3f &orig, const Vec3f &dir, const std::vector<Sphere> &spheres, Vec3f &hit, Vec3f &N, Material &material) {
    ...
    Sphere small_sphere = Sphere(Vec3f( -6, 6, -16.5), 1.5, Material(1.0, Vec4f(0.9, 0.1, 0.0, 0.0), Vec3f(0.15, 0.15, 0.15),   10.));
    Sphere large_sphere = Sphere(Vec3f( -8, 4, -18), 3, Material(1.0, Vec4f(0.9, 0.1, 0.0, 0.0), Vec3f(0.12, 0.12, 0.12),   10.));


    float dist_i_t0;
    float dist_i_t1;
    if (small_sphere.ray_intersect2(orig, dir, dist_i_t0, dist_i_t1) && dist_i_t0 < spheres_dist) {
    ...
    Material duckMaterial( 1.5, Vec4f(0.3,  1.5, 0.2, 0.5), Vec3f(.24, .21, .09),  125. );
    float duck_dist = std::numeric_limits<float>::max();
    for(int nb = 0 ; nb < duck.nfaces() ; nb++) {
        float dist;
        if(duck.ray_triangle_intersect(nb, orig, dir, dist) && dist < spheres_dist && dist < checkerboard_dist) {
            //Recopie la boucle précédente
            duck_dist = dist;
            hit =  orig + dir * dist;
            N = duck.getVecList(nb).normalize();
    ...
    return std::min(duck_dist, std::min(checkerboard_dist, spheres_dist)) < 1000;
}
...
void render(const std::vector<Sphere> &spheres, const std::vector<Light> &lights) {
    ...
    stbi_write_jpg("out_base.jpg", width, height, 3, pixmap.data(), 100);
}
